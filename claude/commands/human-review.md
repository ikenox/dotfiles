...
Read the review output carefully. For each comment, understand the requested change and apply the fix to the corresponding file and line.
After applying all fixes, run git add -N . && git diff | pnpx difit again so the reviewer can verify the changes. Repeat until the reviewer closes the UI with no comments.